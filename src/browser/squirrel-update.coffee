...
binFolder = path.join(rootAppFolder, 'bin')
...
fileKeyPath = 'HKCU\\Software\\Classes\\*\\shell\\GitMaster'
directoryKeyPath = 'HKCU\\Software\\Classes\\directory\\shell\\GitMaster'
backgroundKeyPath = 'HKCU\\Software\\Classes\\directory\\background\\shell\\GitMaster'
applicationsKeyPath = 'HKCU\\Software\\Classes\\Applications\\git-master.exe'
...
installContextMenu = (callback) ->
  ...
  installMenu = (keyPath, arg, callback) ->
    args = [keyPath, '/ve', '/d', 'Open with GitMaster']
    addToRegistry args, ->
...
addCommandsToPath = (callback) ->
  installCommands = (callback) ->
    appCommandPath = path.join(binFolder, 'git-master.cmd')
    relativeAppPath = path.relative(binFolder, path.join(appFolder, 'resources', 'cli', 'git-master.cmd'))
    appCommand = "@echo off\r\n\"%~dp0\\#{relativeAppPath}\" %*"

    appShCommandPath = path.join(binFolder, 'git-master')
    relativeAppShPath = path.relative(binFolder, path.join(appFolder, 'resources', 'cli', 'git-master.sh'))
    appShCommand = "#!/bin/sh\r\n\"$(dirname \"$0\")/#{relativeAppShPath.replace(/\\/g, '/')}\" \"$@\"\r\necho"
    ...
    fs.writeFile appCommandPath, appCommand, ->
...
updateShortcuts = (callback) ->
  if homeDirectory = fs.getHomeDirectory()
    desktopShortcutPath = path.join(homeDirectory, 'Desktop', 'GitMaster.lnk')
...
exports.restartApp = (app) ->
  if projectPath = global.application?.lastFocusedWindow?.projectPath
    args = [projectPath]
  app.once 'will-quit', -> spawn(path.join(binFolder, 'git-master.cmd'), args)
  app.quit()
