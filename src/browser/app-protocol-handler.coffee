...
class AppProtocolHandler
  constructor: (resourcePath, safeMode) ->
    ...
    unless safeMode
      @loadPaths.push(path.join(process.env.APP_HOME, 'dev', 'packages'))

    @loadPaths.push(path.join(process.env.APP_HOME, 'packages'))
  ...
  registerAtomProtocol: ->
    protocol.registerFileProtocol 'app', (request, callback) =>
      ...
      if relativePath.indexOf('assets/') is 0
        assetsPath = path.join(process.env.APP_HOME, relativePath)
