...
class Application
  ...
  @open: (options) ->
    unless options.socketPath?
      if process.platform is 'win32'
        options.socketPath = '\\\\.\\pipe\\git-master-sock'
      else
        options.socketPath = path.join(os.tmpdir(), "git-master-#{options.version}-#{process.env.USER}.sock")
  ...
  openPaths: ({initialPaths, pathsToOpen, executedFrom, pidToKillWhenClosed, newWindow, devMode, safeMode, windowDimensions, profileStartup, window, clearWindowState, addToLastWindow}={}) ->
    ...
    if existingWindow?
      ...
      openedWindow = new AppWindow({initialPaths, locationsToOpen, windowInitializationScript, resourcePath, devMode, safeMode, windowDimensions, profileStartup, clearWindowState})
